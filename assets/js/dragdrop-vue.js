...
var idPage = DragAndDrop.getAttribute("id");
...
new Vue({
  ...
  async mounted() {
    ...
    await this.relaod();
    this.resetLang();
     this.unshuffledQuestion = this.posts[0].items;
  },
  methods: {
    resetLang() {
    ...
    shuffleDrag: async function () {
      ...
      if (this.posts[0].increasedDragOption) {
        this.posts[0].increasedDrag.forEach((increasedDrag) => {
          this.shuffledDrag.push({
    ...
    shuffleQuestion: async function () {
      ...
      console.log(this.unshuffledQuestion);
      let first,
        second,
        temp,
        count = numbers.length;
