...
new Vue({
  ...
  methods: {
    ...
    dragLength() {
      ...
      this.posts[0].slides.forEach((slide) => {
        slide.items.forEach((item) => {
          this.posts[0].dragCounter += item.drag.length;
        });
      });

      this.posts[0].numberOfquestion = this.posts[0].dragCounter;
      console.log("numberOfquestion = " + this.posts[0].numberOfquestion);
      this.numOfSlides = this.posts[0].slides.length;
    },
    updateData() {
      direction != ""
    ...
    drop: function (ev, drag) {
      ...
      if (this.dropID == this.dragNumber) {
        drag.check = true;
        let dragWord =  drag.word 
        this.shuffledDrag.forEach((dg) => {
          dg.forEach((e) => {
            if (e.number == this.dropID && dragWord == e.word) {
              e.check = true;
            }
          });
        });
    ...
    touchEnd: function (event,question) {
      event.preventDefault();
      var element = document.getElementById(this.dragID);
      ...
      if (dropZone != null) {
        if (dropZone.classList.contains("droppable")) {
          ...
          if (this.dropID == this.dragNumber) {
            let dragWord = question.word
            dropZone.style.pointerEvents = "none";
            dropZone.classList.add('ui-droppable-disabled')
            this.posts[0].slides.forEach((slide) => {
              slide.items.forEach((item) => {
                item.drag.forEach((drag) => {
                  if (drag.number == this.dropID && dragWord == drag.word) {
                    drag.check = true;
                  }
                });
              });
            });
            this.shuffledDrag.forEach((drag) => {
              drag.forEach((e) => {
                if (e.number == this.dropID && dragWord == e.word) {
                  e.check = true;
                }
    ...
    swiperPrev: function (index) {
      let counterIndex = index - 1;
      if (index != 0) {
        this.posts[0].slides.forEach((element) => {
          element.isActive = false;
        });
        this.posts[0].slides[counterIndex].isActive = true;
      }
    },
    swiperNext: function (index) {
      let lengthDrag = this.posts[0].slides.length;
      let counterIndex = index + 1;
    ...
    ShuffleDrag: function (num) {
      if (this.counterShow == 0) {
        for (let index = 0; index < num; index++) {
          this.shuffledDrag[index] = [];
          this.posts[0].slides[index].items.forEach((item) => {
            item.drag.forEach((drag, i) => {
              if (!this.shuffledDrag[index]) {
                this.shuffledDrag[index] = [];
              }
              this.shuffledDrag[index].push({
                word: drag.word,
                number: drag.number,
                check: drag.check,
              });
          ...
          let numbers = [...this.shuffledDrag[index]];
          let first,
            second,
            temp,
            count = numbers.length;
          for (let i = 0; i <= this.shuffledDrag[index].length; i++) {
          ...
          this.shuffledDrag[index] = numbers;
        }
        this.counterShow += 1;
      }
    },
