var punctuate = document.querySelector("#punctuate");
var pageUrl = punctuate.getAttribute("data-urlpage");
new Vue({
  el: "#punctuate",
  data: {
    posts: [
  ...
  methods: {
    resetLang() {
      ...
      this.posts[0].quizChooses.forEach((e) => {
        let i = 1;
        while (e[`result${i}`] !== undefined) {
          if (!e[`result${i}`]) {
            e[`chooseUrl${i}`] = "";
          }
          i++;
    ...
    resetTemp() {
      punctuate.insertAdjacentHTML("afterbegin", resetData);
    },
    getDate() {
    ...
    chechedClicked: function (quiz) {
      ...
      if (quiz.type != 4) {
        ...
        quiz[`show${number}`] = !quiz[`show${number}`];
      }
    },
    ...
    chooseAnswer: function (answer, quiz) {
      ...
      if (quiz.type == 1) {
        ...
        quiz["show" + answer.number] = !quiz["show" + answer.number];
        if (answer.answerSelect) {
