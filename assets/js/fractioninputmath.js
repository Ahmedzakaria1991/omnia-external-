  ...
  methods: {
    ...
    showAnswer(item, fraction) {
      ...
      if (item.type == 1) {
        ...
        this.dataContent = event.target.getAttribute("data-content");
        this.dataState = event.target.getAttribute("data-state");
        if (this.dataState == "up") {
          fraction.answerUser[0].answer = fraction.up[0].input[0].show;
          fraction.answerUser[0].help = true;
          fraction.answerUser[0].done = true;
        } else {
          fraction.answerUser[1].answer = fraction.down[0].input[0].show;
          fraction.answerUser[1].help = true;
          fraction.answerUser[1].done = true;
        }
      }
      this.sound_true();
    },
    ...
    inputLength() {
      ...
      //     });
      //     elem.contents[0].dataFractions.forEach((el) => {
      //       this.posts[0].inputCounter += el.up[0].input[0].numperofinput;
      //       this.posts[0].inputCounter += el.down[0].input[0].numperofinput;
      //       // console.log(el.up[0].input[0].numperofinput)
      //     });
      //   });
      // });
    ...
    handleBlur(item, content, fraction) {
      let containerClass = document.querySelector(".container");
      containerClass.scrollTop = 0;
      containerClass.style.cssText = `padding-bottom: 0 ;`;
      this.dataContent = event.target.getAttribute("data-content");
      this.dataState = event.target.getAttribute("data-state");
      let reelBreack = true;
      ...
      if (item.type === 1) {
        ...
        if (this.dataState == "up") {
          fraction.up[0].input[this.dataContent - 1].valid.forEach((elem) => {
            // console.log(elem)
            if (reelBreack) {
              if (this.playerName === elem) {
                event.target.classList.remove("wrong");
                ...
                fraction.answerUser[0].correct = true;

                this.posts[0].counterFinished += 1;
                this.sound_true();
                // content.inputtype2[this.dataContent - 1].valid = content.inputtype2[this.dataContent - 1].valid.filter((e) => e !== elem);
                reelBreack = false;
              } else if (this.playerName !== elem && event.target.value != "") {
                event.target.classList.remove("right");
                event.target.parentNode.classList.remove("right");
          ...
          fraction.down[0].input[this.dataContent - 1].valid.forEach((elem) => {
            if (reelBreack) {
              if (this.playerName === elem) {
                event.target.classList.remove("wrong");
                event.target.parentNode.classList.remove("wrong");
                ...
                fraction.answerUser[1].correct = true;
                this.posts[0].counterCorrect =
                  document.querySelectorAll(".check-input.right").length;
                this.sound_true();
                // content.inputtype2[this.dataContent - 1].valid = content.inputtype2[this.dataContent - 1].valid.filter((e) => e !== elem);
          ...
          fraction.up[0].input[this.dataContent - 1].valid.forEach((elem) => {
            ...
            if (reelBreack) {
              if (this.playerName === elem) {
                ...
                this.playerName.length ===
                fraction.up[0].input[this.dataContent - 1].length
              ) {
                if (this.playerName !== elem && event.target.value != "") {
                  event.target.classList.remove("right");
                  event.target.parentNode.classList.remove("right");
          ...
          fraction.down[0].input[this.dataContent - 1].valid.forEach((elem) => {
            ...
            if (reelBreack) {
              if (this.playerName === elem) {
                ...
                this.playerName.length ===
                fraction.down[0].input[this.dataContent - 1].length
              ) {
                if (this.playerName !== elem && event.target.value != "") {
                  event.target.classList.remove("right");
                  event.target.parentNode.classList.remove("right");
      ...
      this.posts[0].inputSlide.forEach((el) => {
        el.items.forEach((elem) => {
          elem.contents.forEach((elem) => {
            ...
            elem.dataFractions.forEach((element) => {
              bigLength1 = 0;
              bigLength2 = 0;
              element.up[0].input.forEach((elem) => {
                if (bigLength1 < elem.valid[0].length) {
                  bigLength1 = elem.valid[0].length;
                  element.up[0].input[0].length = bigLength1;
                }
                // console.log(element)
              });
              element.down[0].input.forEach((elem) => {
                if (bigLength2 < elem.valid[0].length) {
                  bigLength2 = elem.valid[0].length;
                  element.down[0].input[0].length = bigLength2;
                }
              });
            });
          });
    ...
    popupOpen: function (quiz) {
      ...
      this.posts[0].popuptext = quiz.popup;
      quiz.isActive = true;
    },
    popupClose: function (quiz) {
      this.posts[0].popup = !this.posts[0].popup;
