var inputValidation = document.querySelector("#input-left-images");
var pageUrl = inputValidation.getAttribute("data-urlpage");
new Vue({
  el: "#input-left-images",
  data: {
    posts: [
      {
        id: "",
    ...
    dataContent: 0,
    shufflePopX: null,
    shuffledImagespageX: null,
    shuffledIshufflePopXtems: null,
    shuffledItems: null,
    shufflesildX: null,
    lowerCase: false,
    // --------------------------------------------------------
  ...
  methods: {
    ...
    popupClose: function () {
      ...
      this.posts[0].pop.isActive = false;
      this.popUpState = !this.popUpState;
    },
      ...
      if (direction == "2") {
        await returnData.then((response) => {
          ...
          if (
            ...
            jsonData.items.forEach((el) => {
              el.pages.forEach((item) => {
                // console.log(item);
    ...
    sound_true: function () {
      ...
      // --------------------------------------------------------
      direction != ""
        ? ((this.posts[0].nameId = "input-left-images"),
    ...
    returnLength: function () {
      let bigLength = 0;
      this.posts[0].items.forEach((elemP) => {
        elemP.pages.forEach((elem) => {
    ...
    shuffle: function () {
      ...
      this.posts[0].startTime = this.getDate();
      // --------------------------------------------------------
      let numbers = [...this.posts[0].items[0].pages];

      this.shuffledItems = numbers;
    },
    shuffleImagespage: function () {
      let numbers = [...this.posts[0].items[0].imagepage];
      this.shuffledImagespageX = numbers;
    },
    shufflePop: function () {
      let numbers = [...this.posts];
      this.shufflePopX = numbers;
    },
    shufflesild: function () {
      let numbers = [...this.posts[0].items];
      this.shufflesildX = numbers;
    },
    swiperPrev: function (mySlides, index) {
      ...
      if (index != 0) {
        ...
        this.posts[0].items[counterIndex].isActive = true;
      }
    },
    swiperNext: function (mySlides, index) {
      let lengthDrag = this.posts[0].items.length;
      let counterIndex = index + 1;
