var gtorlt = document.querySelector("#gtorlt");
var pageUrl = gtorlt.getAttribute("data-urlpage");
new Vue({
  el: "#gtorlt",
  data: {
    posts: [
  ...
  methods: {
    resetLang(){
      ...
      resetText.innerHTML += this.posts[0].subject.includes("eng") ? "Do you wish to delete the previous recordings?" : "هل تريد حذف التسجيلات السابقة ؟"
      yes.innerHTML += this.posts[0].subject.includes("eng") ? "Yes" : "نعم"
      no.innerHTML += this.posts[0].subject.includes("eng") ? "No" : "لا"
    },
    resetTemp() {
      gtorlt.insertAdjacentHTML("afterbegin", resetData);
    },
    ...
    clearChecked() {
      this.posts[0].quizChooses.forEach((elem) => {
        elem.chooseUrl != "" && !elem.result ? (elem.chooseUrl = "", elem.show = false): "";
      });
    },
    ...
    chooseAnswer: function (answer, quiz) {
      ...
      this.posts[0].counterCorrect == this.posts[0].quizChooses.length
        ? [
            (this.posts[0].finshedChoose = !this.posts[0].finshedChoose),
            this.finshed(),
          ]
