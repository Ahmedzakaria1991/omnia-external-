var choose = document.querySelector("#choose-slide");
var pageUrl = choose.getAttribute("data-urlpage");
new Vue({
  el: "#choose-slide",
  data: {
    posts: [
  ...
  methods: {
    ...
    resetTemp() {
      choose.insertAdjacentHTML("afterbegin", resetData);
    },
    getDate() {
    ...
    getData: async function () {
      ...
      !this.dataLoaded
        ? await fetch("../json/" + pageUrl + ".json")
            ...
            .then((data) => {
              this.posts = data;
              this.posts[0].chooseSlide.forEach((element) => {
      ...
      this.posts[0].counterCorrect == this.posts[0].numberOfquestion
        ...
        : "";
      console.log(
        "this.posts[0].numberOfquestion = " + this.posts[0].numberOfquestion
      );
    ...
    clearChecked() {
      this.posts[0].chooseSlide.forEach((element) => {
        element.quizChooses.forEach((el) => {
          el.answers.forEach((elem) => {
            !elem.right ? (elem.checked = elem.right) : "";
          });
        });
      });
    },
    ...
    updateData() {
      direction != ""
        ? ((this.posts[0].nameId = "choose-slide"),
    ...
    finshed: function () {
      ...
      this.posts[0].finshedChoose = true;
      setTimeout(() => {
        // console.log(this.posts[0].finshedChoose);
        finalResponse.submitData(
          JSON.stringify(this.posts),
          4,
          this.posts[0].counterCorrect
    ...
    chooseLength: function () {
      this.posts[0].chooseSlide.forEach((element) => {
        this.quizLength += element.quizChooses.length;
      });
      // console.log("quizLength = " + this.quizLength);
    },
    chechedClicked: function (answer, quiz) {
      quiz.answers.forEach((elem) => {
        elem.checked = false;
      });
      ...
      if (quiz.correctAnswer.length == 1) {
        quiz.correctAnswer.forEach((i, index) => {
          if (i === answer.numAnswer) {
            answer.right = !answer.right;
            quiz.counter += 1;
            quiz.answerSelect = !quiz.answerSelect;
        ...
        quiz.correctAnswer.forEach((i, index) => {
          if (breakLoop) {
            if (i === answer.numAnswer) {
              ...
              if (quiz.counter === quiz.correctAnswer.length) {
                quiz.answerSelect = !quiz.answerSelect;
                this.posts[0].counterCorrect += 1;
                this.updateData();
                breakLoop = false;
              ...
              countFalse++;
              countFalse == quiz.correctAnswer.length
                ? this.sound_false()
                : false;
            }
          }
      ...
      this.posts[0].counterCorrect == this.quizLength
        ? this.finshed()
        : this.posts[0].finshedChoose;
      // this.posts[0].counterCorrect == this.posts[0].quizChooses.length ? this.finshed() : this.posts[0].finshedChoose;
    },
    ...
    shuffle: async function () {
      ...
      for (let i = 0; i <= this.posts[0].quizChooses.length; i++) {
    ...
    slide: async function () {
      ...
      // --------------------------------------------------------
      let numbers = [...this.posts[0].chooseSlide];
      this.slideQ = numbers;
    },
    stopsound() {
    ...
    swiperPrev: function (mySlides, index) {
      ...
      if (index != 0) {
        ...
        this.posts[0].chooseSlide[counterIndex].active = true;
      }
      this.stopsound();
    },
    swiperNext: function (mySlides, index) {
      let lengthInput = this.posts[0].chooseSlide.length;
      let counterIndex = index + 1;
    ...
    popupClose: function () {
      // console.log("this.quizLocAction = " + this.quizLocAction)
      this.posts[0].popup = !this.posts[0].popup;
      this.popUpState = !this.popUpState;
    ...
    passageOpen: function (mySlides) {
      ...
      this.posts[0].popuptext = mySlides.popuptext;
      // Change Here  -------------------------------------------
      this.popUpState = !this.popUpState;
