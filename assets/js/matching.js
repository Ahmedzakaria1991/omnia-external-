...
var columnLeft = document.querySelector(".columnLeft .point");
var numOfPointer = [];
var classListNames = [];
  ...
  methods: {
    ...
    getUnits: async function () {
      ...
      function finshedPopUp() {
        ...
        // console.log("finshedPopUpX = " + finshedPopUpX)
      }
      $(".cointDrag .point").mousemove(function (event) {
        var idLineSelected = globalIdReturn(event);
        $("#" + idLineSelected).attr({
          x2: event.pageX,
          y2: event.pageY,
      ...
      function creatSvgDrag(event, _position, container) {
        ...
        if (
          $("#Conect").hasClass("eng-4") ||
          $("#Conect").hasClass("eng-5") ||
          $("#Conect").hasClass("eng-6") ||
          $("#Conect").hasClass("eng")
        ) {
          if ($(event.target).hasClass("flip")) {
            $("#" + idLineSelected).attr({
              x1: _position.left,
              y1: _position.top + $(container).height() / 2,
              x2: _position.left,
              y2: _position.top + $(container).height() / 2,
            });
          } else {
            $("#" + idLineSelected).attr({
              x1: _position.left + $(container).width(),
              y1: _position.top + $(container).height() / 2,
              x2: _position.left + $(container).width(),
              y2: _position.top + $(container).height() / 2,
            });
          }
        } else {
    ...
    myEventHandler: function () {
      ...
      for (let i = 1; i <= pointLength; i++) {
        ...
        let pointLeft = $(".columnLeft .point[data-number=" + i + "]");
        ...
        if (
          ...
          $("#lineSVG" + i + "").attr({
            x1: parentPointRight.offset().left + pointLeft.width(),
            y1: parentPointRight.offset().top + pointRight.height() / 2,
            x2: parentPointLeft.offset().left,
            y2: parentPointLeft.offset().top + pointRight.height() / 2,
          });
        } else {
          $("#lineSVG" + i + "").attr({
            x1: parentPointRight.offset().left,
            y1: parentPointRight.offset().top + pointRight.height() / 2,
            x2: parentPointLeft.offset().left + pointLeft.width(),
            y2: parentPointLeft.offset().top + pointRight.height() / 2,
          });
        }
      }
                  ...
                  if (numberSet >= 0) {
                    $("#" + className).attr({
                      ...
                      y1:
                        ...
                        $("." + className).height() / 2,
                      x2: $(classLeftName).offset().left,
                      y2:
                        $(classLeftName).offset().top +
                        $(classLeftName).height() / 2,
                    });
                  }
                } else {
                  if (numberSet >= 0) {
                    $("#" + className).attr({
                      x1: $("." + className).offset().left,
                      y1:
                        $("." + className).offset().top +
                        $("." + className).height() / 2,
                      x2:
      ...
      classListNames.forEach((className) => {
        ...
        if (numberSet >= 0) {
          $("#" + className).attr({
            x1: $("." + className).offset().left,
            y1:
              $("." + className).offset().top + $("." + className).height() / 2,
            x2: $(classLeftName).offset().left + $(classLeftName).width(),
            y2: $(classLeftName).offset().top + $(classLeftName).height() / 2,
          });
        }
      });
    },
