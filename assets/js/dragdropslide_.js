var DragAndDrop = document.querySelector("#DragDropSlide");
...
new Vue({
  ...
  methods: {
      ...
      if (direction == "2") {
        await returnData.then((response) => {
          ...
          if (
            ...
            this.posts[0].SubType == jsonData.nameId
    ...
    dragLength() {
      this.posts[0].daragSlid.forEach((el, index) => {
        el.droggableBox[0].pagequestion[0].question[0].addQ.forEach((ele) => {
          this.posts[0].dragCounter += ele.number.length;
        });
      });
    ...
    getUnits: async function () {
      ...
      var DragAndDropSlide = document.querySelector("#DragDropSlide");
      var pageUrl = DragAndDropSlide.getAttribute("data-urlpage");
    ...
    shuffle: async function () {
      let numbers = [
        ...this.posts[0].daragSlid[0].droggableBox[0].pagequestion[0]
          .question[0].addQ,
      ];
      ...
      for (
        let i = 0;
        i <= this.posts[0].daragSlid[0].droggableBox.length;
        i++
      ) {
    ...
    // shufflanswer: async function() {
    //     let numbers = [...this.posts[0].daragSlid[0].droggableBox[0].pagequestion[0].answer];

    //     let first,
    //         second,
    //         temp,
    //         count = numbers.length;
    //     for (let i = 0; i <= this.posts[0].daragSlid[0].droggableBox[0].pagequestion[0].answer.length; i++) {
    //         first = Math.floor(Math.random() * count);
    //         second = Math.floor(Math.random() * count);
    //         temp = numbers[first];
    //         numbers[first] = numbers[second];
    ...
    shuffledQ: async function () {
      let numbers = [...this.posts[0].daragSlid];
      // let first,
      //     second,
      //     temp,
      //     count = numbers.length;
      // for (let i = 0; i <= this.posts[0].daragSlid.length; i++) {
      //     first = Math.floor(Math.random() * count);
      //     second = Math.floor(Math.random() * count);
      //     temp = numbers[first];
      //     numbers[first] = numbers[second];
      ...
      // console.log(this.posts[0].daragSlid)
    },
    swiperPrev: function (mySlides, index) {
      // this.posts[0].counterFinished = 0;
      let counterIndex = index - 1;
      if (index != 0) {
        this.posts[0].daragSlid.forEach((element) => {
          element.isActive = false;
        });
        this.posts[0].daragSlid[counterIndex].isActive = true;
      }
    },
    swiperNext: function (mySlides, index) {
      // this.posts[0].counterFinished = 0;
      let lengthDrag = this.posts[0].daragSlid.length;
      let counterIndex = index + 1;
      if (lengthDrag != counterIndex) {
        this.posts[0].daragSlid.forEach((element) => {
          element.isActive = false;
        });
        this.posts[0].daragSlid[counterIndex].isActive = true;
    ...
    confirm: function (ui) {
     ...
     relaod: async function () {
      ...
      if (direction == "2") {
        // console.log(this.data)
        if (this.data.learningObjectAsJson != "" && this.data.learningObjectAsJson != null) {
          if (JSON.parse(this.data.learningObjectAsJson).pointCorrect.length != 0) {
            document.querySelectorAll('.active .droppable').forEach(element => {
              ...
              JSON.parse(this.data.learningObjectAsJson).pointCorrect.forEach(el => {
                ...
                document.querySelectorAll('.active .draggable').forEach(ele => {
