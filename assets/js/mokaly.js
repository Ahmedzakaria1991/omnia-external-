var mokaly = document.querySelector("#mokaly");
var pageUrl = mokaly.getAttribute("data-urlpage");
new Vue({
  el: "#mokaly",
  data: {
    posts: [
  ...
  methods: {
    ...
    getData: async function () {
      ...
      !this.dataLoaded
        ? await fetch("../json/" + pageUrl + ".json")
            ...
            .then((data) => {
              this.posts = data;
              this.posts[0].numberOfquestion = null;
              console.log(
        ...
        : "";
      // --------------------------------------------------------
      this.posts[0].quizChooses.length != 0
        ? setTimeout(() => {
            this.isLoading = true;
    ...
    shuffle: function () {
      let numbers = [...this.posts[0].quizChooses[0].questions];
      let first,
        second,
        temp,
        count = numbers.length;
      for (let i = 0; i <= this.posts[0].quizChooses[0].questions.length; i++) {
    ...
    swiperPrev: function (mySlides, index) {
      ...
      if (index != 0) {
        this.posts[0].quizChooses.forEach((element) => {
          element.isActive = false;
        });
        this.posts[0].quizChooses[counterIndex].isActive = true;
      }
    },
    swiperNext: function (mySlides, index) {
      // this.posts[0].counterFinished = 0;
      let lengthDrag = this.posts[0].quizChooses.length;
      let counterIndex = index + 1;
