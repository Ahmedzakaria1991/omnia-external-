...
checkAnswer() {
  ...
  const correctInputsFromRows = [];
  item.rows.forEach((row) => {
    row.forEach((cell) => {
      if (cell.kind === "input") {
        correctInputsFromRows.push(cell.valid);
      }
    });
  });

  for (let i = 0; i < correctInputsFromRows.length; i++) {
    const expected = normalize(correctInputsFromRows[i]);
    const actual = normalize(this.inputs[i]);

    console.log(`Checking input ${i}: expected = "${expected}", actual = "${actual}"`);
  ...
  //  التحقق من النتيجة النهائية (اختياري)
