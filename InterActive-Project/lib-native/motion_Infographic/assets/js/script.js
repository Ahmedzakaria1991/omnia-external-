const { createApp } = Vue;
import { loadTimerAnimation, loadtransitionAnimation } from "./animation.js";
...
createApp({
  ...
  async mounted() {
    ...
    const Transition = loadtransitionAnimation();

    window.animations = { Timer, Transition };
    ...
    this.failAudio.src =
      "../../../lib-native/motion_Infographic/assets/audio/fail.mp3";
    this.successAudio.src =
      "../../../lib-native/motion_Infographic/assets/audio/success.mp3";
    ...
    playerInstance.load(source).then(() => {
      ...
      playerInstance.on(playerInstance.exports.PlayerEvent.TimeChanged, () => {
        // For Get Current Second Watch Live on Video.
        const current = Math.floor(playerInstance.getCurrentTime());

        // For Store the second where question put
        const popupTimes = this.posts[0].popupTimes;

        // For store second question solved
        ...
        if (shouldShowQuestion) {
          ...
          this.quizvideo = true;
          this.questionCounter = popupTimes.indexOf(current); // /overwrite 1 ==> 0
          document.getElementById("interaction").style.display = "flex";
          animations.Transition.playSegments([1, 100], true);
          setTimeout(() => {
            animations.Timer.destroy();
            this.activeQuestion = this.posts[0].questions[this.questionCounter]; // ==> question[0]
            this.activeQuestion.quiz[this.quizCounter].startTime =
              this.getDate();
            this.activeQuestion.quiz[this.quizCounter].add_style = "";
            this.activeQuestion.quiz[this.quizCounter].active = true;
            animations.Transition.playSegments([0, 0], true);
            animations.Transition.addEventListener("complete", () => {
              animations.Transition.destroy();
            });
            let choose = document.getElementsByClassName("choose");
            Array.from(choose).forEach((e) => {
              e.classList.add(
                `length${this.activeQuestion.quiz[this.quizCounter].chooses.length}`,
              );
            });
            setTimeout(() => {
  ...
  methods: {
    ...
    playSound(e, soundPath) {
      e.stopPropagation();
      this.questionAudio.pause();
      this.soundAudio.src = soundPath;
    ...
    playLo: async function (type) {
      this.posts[0].questions[this.questionCounter].active = true;
      this.activeQuestion = this.posts[0].questions[this.questionCounter];

      console.log(this.activeQuestion);
      console.log(`Question Coutner ==>  ${this.questionCounter}`);

      // For Inizialize Drag Question (Type 3)
      if (this.activeQuestion.quiz[this.quizCounter].type == 3) {
        console.log("Drag Question Turn On");
    ...
    // For Responsible for click next on popup
    next: async function () {
      this.expressionAudio.pause();
      this.feedback = false;

      console.log(
        `quizCounter = ${this.quizCounter}, and active question Length = ${this.activeQuestion.quiz.length}`,
      );

      if (this.quizCounter < this.activeQuestion.quiz.length - 1) {
        console.log("True Condition Activeed In Next Fun");
        ...
        console.log("False Condition Activeed In Next Fun");
    ...
    Resize: async function () {
      ...
      if (screenW / screenH > 16 / 9) {
        // الشاشة أعرض من 16:9 → احسب على الطول
        height = screenH;
        width = (height * 16) / 9;
      } else {
        // الشاشة أطول → احسب على العرض
        width = screenW;
        height = (width * 9) / 16;
      }
      const borderQuestion = document.getElementById("border");
      borderQuestion.style.height = `${height - padding}px`;
      borderQuestion.style.width = `${width - padding}px`;
    },
    ...
    dragstart(ev) {
      ...
      this.dragTransforms[id] = this.dragTransforms[id] || { x: 0, y: 0 };
    ...
    drag(ev) {
      ...
      this.dragTransforms[this.dragID] = { x: dx, y: dy };
    },
    ...
    drop(ev) {
      ...
      let dropZone = ev.target.closest(".droppable");
      if (!dropZone) {
        this.returnToOriginal(draggedEl, draggedId);
        return;
      }

      const existingItem = dropZone.querySelector(".draggable");
      if (existingItem && existingItem !== draggedEl) {
        this.returnToOriginal(existingItem, existingItem.id);
      }

      dropZone.appendChild(draggedEl);

      this.dragTransforms[draggedId] = { x: 0, y: 0 };
      draggedEl.style.pointerEvents = "auto";
      draggedEl.style.zIndex = "10";

      this.checkIfAllDropped();
    },
    ...
    touchStart(event) {
      ...
      this.dragNumber = target.getAttribute("index");
    ...
    touchMove(event) {
      ...
      el.style.left = event.touches[0].clientX - this.startX + "px";
      el.style.top = event.touches[0].clientY - this.startY + "px";
    },

    touchEnd(event) {
      if (!this.dragID) return;

      const draggedEl = document.getElementById(this.dragID);
      if (!draggedEl) return;
      ...
      if (!dropZone) {
        this.returnToOriginal(draggedEl, this.dragID);
        this.dragID = null;
        return;
      }
      ...
      this.dragTransforms[this.dragID] = { x: 0, y: 0 };
      draggedEl.style.pointerEvents = "auto";
      draggedEl.style.zIndex = "10";
    ...
    returnToOriginal(el, id) {
      if (!el) return;

      const originalContainer = document.getElementById("card-dragg");
      if (!originalContainer) return;

      originalContainer.appendChild(el);

      this.dragTransforms[id] = { x: 0, y: 0 };
      el.style.pointerEvents = "auto";
    ...
    //               helper functions
    // ────────────────────────────────────────────────
    resetDraggedElement(id) {
      if (this.dragTransforms[id]) {
        this.dragTransforms[this.dragID] = { x: 0, y: 0 };
      }
    ...
    checkIfAllDropped() {
      const allDropZones = document.querySelectorAll(".droppable");
      const filledCount = Array.from(allDropZones).filter((zone) =>
        zone.querySelector(".draggable"),
      ).length;

      if (filledCount === allDropZones.length) {
    ...
    checkFinalAnswer() {
      ...
      allDropZones.forEach((zone) => {
        ...
        const zoneIndex = zone.getAttribute("index");
        zone.classList.remove("correct-temp", "wrong-temp");
      ...
      this.trueAnswer(
        this.activeQuestion.quiz[this.quizCounter],
        correctCount,
        allDropZones.length,
      );
