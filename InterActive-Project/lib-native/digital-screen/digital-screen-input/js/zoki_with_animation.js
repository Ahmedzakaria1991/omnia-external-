...
new Vue({
  ...
  async mounted() {
    ...
    // أضف originalIndex لكل سؤال بعد الخلط
    this.posts[0].items.forEach((item, index) => {
      // ابحث عن ترتيبه الأصلي في data.json
      const original = this.posts[0].items_original?.find(
        (q) => q.id === item.id
      );
      item.originalIndex = original
        ? this.posts[0].items_original.indexOf(original)
        : index;
    });

    // تهيئة أنيميشن السهم بعد تحميل البيانات
    if (window.ArrowAnimation) {
      window.ArrowAnimation.init();
    }
    // this.numOfQuestions();
    this.bgAudio.src =
  ...
  methods: {
    ...
    getData: async function () {
      ...
      !this.dataLoaded
        ? await fetch(pageUrl + ".json")
            ...
            .then((data) => {
              this.posts = data;
              this.posts[0].items_original = [...this.posts[0].items];
              // console.log(
    ...
    nextQuestion() {
      ...
      if (this.loQuestions != this.counter) {
        ...
        const originalIndex = this.activeQuestion.originalIndex;
        if (window.ArrowAnimation && originalIndex !== undefined) {
          window.ArrowAnimation.goTo(originalIndex);
        }
    ...
    checkanswer() {
      ...
      const Success = checkInputALL(this.activeQuestion);
      document.querySelector(".check-button")?.classList.add("disable");

      if (Success) {
        const originalIndex = this.activeQuestion.originalIndex;
        if (window.ArrowAnimation && originalIndex !== undefined) {
          window.ArrowAnimation.play(originalIndex); // الأنيميشن الصحيح
        }
        zokicharecter.playSegments([65, 80], true);
        this.rightAnswer.play(),
          // console.log(this.activeQuestion.holdTime),
          setTimeout(() => this.trueAnswer(), this.activeQuestion.holdTime);
      } else {
        this.falseAnswer();
      }
    ...
    showAnswer() {
      ...
      const originalIndex = this.activeQuestion.originalIndex;
      if (window.ArrowAnimation && originalIndex !== undefined) {
        window.ArrowAnimation.play(originalIndex);
      }
