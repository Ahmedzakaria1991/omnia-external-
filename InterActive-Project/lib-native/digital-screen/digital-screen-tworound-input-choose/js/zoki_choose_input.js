...
new Vue({
 ...
 addFont() {
      ...
      if (this.loQuestions != this.counter) {
        ...
        setTimeout(() => {
          if (
            ...
            this.counter != this.posts[0].numberOfquestion - 1
          ) {
            this.narrationsound();
          }
        }, 2500);
        ...
        document
          .querySelector(".question.active .check-button")
          ?.classList.add("disable");
        ...
        document.querySelectorAll(".question.active .slice").forEach((el) => {
          el.style.userSelect = "auto";
          el.style.pointerEvents = "auto";
        });
      } else {
    ...
    checkanswer() {
      ...
      document.querySelector(".check-button")?.classList.add("disable");
      document.querySelectorAll(".question.active .square").forEach((el) => {
        if (el) {
      ...
      if (this.roundTwo) {
        ...
        if (
          this.choosecounter == this.posts[0].items[this.counter].number_checked
        ) {
          this.trueAnswerChoose();
        } else {
          this.falseAnswerChoose();
        }
      }
    },
    showAnswer() {
      ...
      if (this.roundTwo) {
        ...
        document.querySelectorAll(".question.active .checked").forEach((el) => {
          el?.classList.remove("checked");
        });
        document.querySelectorAll(".question.active .square").forEach((el) => {
          plus++;

          if (plus > this.posts[0].items[this.counter].number_checked) {
    ...
    multipleChoose(event, i) {
      event.target.classList.contains("checked")
        ? [event.target.classList.remove("checked"), this.choosecounter--]
        : [event.target.classList.add("checked"), this.choosecounter++];
      this.choosecounter == 0
        ? document.querySelector(".check-button")?.classList.add("disable")
        : document.querySelector(".check-button")?.classList.remove("disable");
      this.helpHand_shape = false;
      this.clickBtn.play();
      this.reset();
      this.questionSound(this.zokiStatus);
      this.clickBtn.play();
    ...
    getSlicePath(index) {
      const total = 6; // عدد الأجزاء الثابت
